 ...
 * @param {*} param1 
 */
export function uniDialog(content, { isCancel = false, title = '提示', confirmText = '确认', cancelText = '取消', ...moreObj } = {}) {
 ...
 * @param {*} options 扩展参数
 */
export function uniToast(content, { position = 'center', duration = 2000, isMask = true, icon = 'none', ...moreObj } = {}) {
  if (content) {
    uni.showToast({
      title: content,
      position: position,
 ...
 * @param {*} options 
 */
export function uniLoading(content, { isMask = true, ...moreObj } = {}) {
  if (content) {
    uni.showLoading({
      title: content,
      mask: isMask,
  ...
  return new Promise((resolve) => {
    ...
    uni.authorize({
      ...
      async fail(authorizeErr) {
        ...
        const result = await uniDialog(tips || '我们需要您提供相关系统权限,才能使用相关服务,是否手动开启?', { isCancel: true });
        if (result) {
          uni.openSetting({
  ...
  return new Promise((resolve) => {
    uni.login({
      ...
      success({ authResult, code, errMsg }) {
        ...
        resolve({ authResult, code, errMsg, status: true });
      },
      fail() {
 ...
 * @param {*} options 
 */
export async function uniPreviewImage(list = [], { index = 0, name = 'url', loop = true, actionList = [], actionName = 'name', ...mixins } = {}) {
  ...
  return new Promise(resolve => {
    uni.chooseImage({
      ...
      success: ({ tempFilePaths, tempFiles }) => {
        ...
        resolve({ status: true, localPathFileArr: tempFilePaths, formDataFileArr: tempFiles });
      },
      fail: (err) => {
  ...
  return new Promise((resolve) => {
    uni.chooseLocation({
      success(res) {
        ...
        resolve({ status: true, address, latitude, longitude, name });
      },
      fail(err) {
        console.log('uni.chooseLocation.err', err);
        resolve({ status: false, address: "", latitude: 0, longitude: 0 });
      },
      ...moreOptions
    });
  })
...
export function mapMethod(func) {
  return func();
};
